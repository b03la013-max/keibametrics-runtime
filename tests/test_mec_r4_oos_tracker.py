...
def test_mec_r4_oos_counts_only_preregistered_formal_pre_race(tmp_path, monkeypatch):
    ...
    _write(f"runtime/final_artifacts/{rid2}.json",{"sha256":"FINAL2"})
    ...
    _write(f"runtime/final_artifacts/{rid3}.json",{"sha256":"FINAL3"})
...
def test_arm_set_mismatch_is_rejected(tmp_path, monkeypatch):
    ...
    _write(f"runtime/final_artifacts/{rid}.json",{"sha256":"F"})
